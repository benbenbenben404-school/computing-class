...
function load_data(time) {
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (!current_user) {
        current_user =
          data.getElementsByTagName("username")[0].childNodes[0].nodeValue;
      }

      if (
        data.getElementsByTagName("time_returned")[0].childNodes[0].nodeValue <=
        last_data_time
      ) {
        //return
      }
      ...
      for (i = 0; i < chats.length; i++) {
        ...
        let chat_id =
          chat.getElementsByTagName("chat_id")[0].childNodes[0].nodeValue;
        ...
        if (!(chat_id in global_data["chats"])) {
          new_chat = {
            ...
            "unread-messages": 0,
            owner:
              chat.getElementsByTagName("owner")[0].childNodes[0].nodeValue,
            "chat-name":
              chat.getElementsByTagName("name")[0].childNodes[0].nodeValue,
            users: [],
          };
        ...
        for (j = 0; j < messages.length; j++) {
          ...
          if (
            ...
            localStorage.getItem("last_returned") < mes_time
          ) {
...
function update_side_panel() {
  ...
  for (let chat_id in global_data["chats"]) {
    ...
    descriptor.querySelectorAll("p")[0].innerHTML =
      global_data["chats"][chat_id]["chat-name"];
    ...
    if (global_data["chats"][chat_id]["unread-messages"] != 0) {
      ...
      descriptor.querySelectorAll("span")[0].innerHTML =
        global_data["chats"][chat_id]["unread-messages"];
    }
...
function switch_panel(panel_selector) {
  ...
  if (
    current_panel &&
    document.querySelector(".side-window") != current_panel
  ) {
    ...
    if (current_user == owner && user != owner) {
      ...
      user_display.querySelector(".small-button").addEventListener(
        ...
        function () {
          ...
          global_data["chats"][chat_id]["users"].splice(
            global_data["chats"][chat_id]["users"].indexOf(user),
            1
          );
...
function show_chat(chat_id) {
  ...
  for (let message in global_data["chats"][chat_id]["messages"]) {
    let mes_content =
      global_data["chats"][chat_id]["messages"][message]["content"];
...
function send_message() {
  ...
  if (content) {
    ...
    document
      ...
      .setCustomValidity("Must not be blank");
  }
...
function create_chat() {
  ...
  if (!chat_name) {
    document.querySelector("#create_chat_validation").innerHTML =
      "Chat name must not be blank";
...
function live_search(search_field, callback) {
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      var usernames_returned =
        xhttp.responseXML.getElementsByTagName("username");
